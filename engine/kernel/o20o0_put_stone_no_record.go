...
func executeSubroutine(k *Kernel, placePlay Point, color Stone, ls *LibertySearchAlgorithm, libInfo *LibertyInfo) int {
	...
	if ls.board.IsMasonry(placePlay) { // 石の上に石を置こうとしたか
		return 4
	}
	...
	var eachAdjacentPointWhenCaptureStone = func(dir Cell_4Directions, p Point) {
		...
		if adjColor == oppColor && // 隣接する連が相手の石で（枠はここで除外されます）
			...
			!ls.board.IsSpaceAt(p) { // 石はまだあるなら（上と右の石がくっついている、といったことを除外）

			ls.board.FillRen(p, oppColor)
	...
	ls.board.ForeachNeumannNeighborhood(placePlay, eachAdjacentPointWhenCaptureStone)

	ls.board.SetStoneAt(placePlay, color)
	ls.findRen(placePlay)
...
func PutStone(k *Kernel, placePlay Point, color Stone) int {
	...
	return executeSubroutine(k, placePlay, color, ls, &libInfo)
}
